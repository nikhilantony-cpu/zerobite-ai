...
export function AiChatAssistant() {
  const { language, meals, vendors, predictions } = useApp();
    ...
    setTimeout(() => {
      ...
      if (lower.includes("biryani") || lower.includes("non veg") || lower.includes("chicken")) {
        ...
        if (biryani && biryani.availableQuantity > 0) {
          ...
          botResponse = "Ah, it looks like today's Biryani surplus is completely sold out or not listed yet. Check back around 5:00 PM!";
        }
      } else if (lower.includes("veg") || lower.includes("thali") || lower.includes("samosa") || lower.includes("healthy")) {
      ...
      } else if (lower.includes("predict") || lower.includes("waste") || lower.includes("rain") || lower.includes("tomorrow") || lower.includes("fewer")) {
        botResponse = "🤖 Smart AI Prediction: Based on tomorrow's weather forecast (Heavy Rain, 28°C) and Onam festival holidays, campus footfall is predicted to drop by 18%. Our FastAPI neural network advises cafeterias to prepare 35 fewer meals tomorrow to maintain zero leftover risk!";
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed right-6 bottom-6 z-50 bg-gradient-to-br from-green-600 via-green-700 to-emerald-800 text-white p-4 rounded-3xl shadow-2xl flex items-center gap-3 hover:scale-105 hover:shadow-green-700/50 transition-all duration-300 border-2 border-white/40 cursor-pointer ${
          isOpen ? "hidden" : "block"
        }`}
      >
          ...
          <div className="bg-green-50/80 p-2.5 border-b border-green-100 flex items-center gap-2 overflow-x-auto text-xs">
            {[
              ...
              <button
                ...
                className="bg-white text-green-900 hover:bg-green-700 hover:text-white border border-green-200 px-3 py-1 rounded-full font-bold whitespace-nowrap transition shadow-2xs cursor-pointer shrink-0"
              >
                {faq}
              </button>
            ))}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-slate-50 to-white">
            {messages.map((m, idx) => (
              ...
              >
                ...
                <div
                  className={`p-4 rounded-3xl max-w-[82%] text-xs leading-relaxed font-medium shadow-xs ${
                    m.sender === "user"
                ...
                >
                  ...
                  <span
                    className={`block text-[9px] mt-2 ${
                      m.sender === "user" ? "text-green-200 text-right" : "text-slate-400 text-left font-bold"
                    }`}
                  >
                    {m.time}
                  </span>
